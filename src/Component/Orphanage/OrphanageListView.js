...
function OrphanageListView() {
  ...
  return (
    <div>
          <section>
        <h1 class='mb-3' style={{textAlign:'center',color:'brown',marginTop:'50px'}}>Listed Orphanages</h1>
          ...
          <tbody>
            {data.map((data, index) => (
              <tr key={data._id}>
                ...
                <td><Link to={`/orphanagedetails/${data._id}`}><button type="submit" class="btn btn-info border-0" style={{backgroundColor:'brown',color:'white'}}>View More</button></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default OrphanageListView